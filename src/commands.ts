...
type Command = {
    keyword: string;
    description: string;
    optionsBuilder: (builder: SlashCommandBuilder) => Partial<SlashCommandBuilder>;
...
export const GURPS: Command = {
    ...
    optionsBuilder: (builder) => builder.addStringOption((option) =>
        option
            .setName("input")
...
export const ShadowRun: Command = {
    ...
    handler: async (interaction) => {
        ...
        const glitch = values.filter(value => value === 1).length >= values.length / 2 ? "### GLITCH DETECTED ###\r\n\r\n" : "";
...
export const NotePassing: Command = {
    ...
    handler: async (interaction) => {
        ...
        await ourThread.send(`<@${interaction.user.id
...
export const Par: Command = {
    ...
    optionsBuilder: (builder) => builder.addStringOption((option) =>
        option
            .setName("input")
            .setDescription(
                "Format: numberOfRolles,successesNeeded (Example: 5,3)"
            )
            .setRequired(true)
    ),
    handler: async (interaction) => {
